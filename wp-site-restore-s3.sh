...
VERSION="v23-debug"
# v23-debug: ВРЕМЕННО отключено удаление архива после скачивания для проверки целостности
#            Архив остается в /backup/ для ручной проверки
# v22: Добавлена проверка и автоматическое добавление расширения .tar к файлу, если его нет
#      v-restore-user требует, чтобы файл имел расширение .tar для правильной работы
# v21: Убрана логика создания нового архива - теперь передаем оригинальный полный бэкап в v-restore-user
#      v-restore-user сам извлечет нужный домен по параметру WEB=domain
# v20: ИСПРАВЛЕНИЕ вызова v-restore-user для работы с полным бэкапом пользователя HestiaCP
#      Параметры: WEB=domain (восстановить только указанный домен), DNS/MAIL/DB/CRON/UDIR='no' (пропустить)
#      БД не восстанавливается через v-restore-user, т.к. скрипт создает БД и импортирует дамп вручную позже
...
#   DB='no' - НЕ восстанавливать БД (создадим и импортируем вручную позже из wp-config.php)
...
if [ "${RESTORE_FAILED:-0}" -ne 0 ]; then
    ...
    # ВРЕМЕННО ОТКЛЮЧЕНО: Удаляем все временные файлы при ошибке
    # Нужно для проверки целостности скачанного архива
    ORIGINAL_BACKUP=$(basename "$FULL_S3_PATH")
    echo "ВНИМАНИЕ: Архив НЕ удален для проверки: $BACKUP_DIR/$BACKUP_FILE" | tee -a "$LOG_FILE"
...
# === ВРЕМЕННО ОТКЛЮЧЕНО: Удаляем архив при успешном восстановлении ===
# Нужно для проверки целостности скачанного архива
log_progress "Очистка временных файлов (удаление архива ОТКЛЮЧЕНО)..."

ORIGINAL_BACKUP=$(basename "$FULL_S3_PATH")
echo "ВНИМАНИЕ: Архив НЕ удален для проверки: $BACKUP_DIR/$BACKUP_FILE" | tee -a "$LOG_FILE"
echo "Можете проверить архив вручную: ls -lh $BACKUP_DIR/" | tee -a "$LOG_FILE"
